...
export default async function CardPage({ params }: { params: Promise<{ cardId: string }> }) {
  ...
  const [card, listingsRes] = await Promise.all([
    ...
    supabase
      .from('listings')
      .select('*, profiles(id, username, avatar_url, display_name, created_at, messenger_link), binders(id, name)')
      .eq('card_id', cardId)
      .eq('status', 'listed')
              ...
              {(refUsd || refFoil) && (
                <div className="cdp-ref-prices" style={{ padding: '14px 16px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '12px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                    ...
                    {refFoil && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '11px', color: '#fbbf24', display: 'inline-flex', alignItems: 'center', gap: '3px' }}><svg width="10" height="10" viewBox="0 0 24 24" fill="#fbbf24" stroke="#fbbf24" strokeWidth={1}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg> Foil</span>
